...
# Dossier commun pour les fichiers de définitions et de pistes
...
def charger_infos_auteur(chemin_json: str) -> dict:
    """Récupère les informations de l'auteur depuis le JSON."""
    data = charger_json(chemin_json)
    return data.get("auteur", {})
...
def ecrire_page_informations(infos: dict):
    """Génère le contenu de la page d'informations à partir du dictionnaire infos."""
    nom = infos.get("nom", "")
...
def generer_latex_depuis_markdown(contenu_markdown: str, llm: Llama) -> str:
    """
    Convertit rigoureusement un contenu markdown en document LaTeX structuré.
    Aucune invention de contenu : chaque élément du markdown doit être reproduit textuellement.
    """
    prompt = (
